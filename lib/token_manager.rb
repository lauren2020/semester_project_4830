...
class TokenManager
  def self.generate_token(user_id)
    expire_time = Time.now + 1.day #2*60*60
  ...
  def self.gen_enc_token(user_id, create_time)
    ...
    puts "GenToken"
    puts SemesterProject4830::Application.credentials.secret_key_base

    OpenSSL::HMAC.hexdigest(
      'sha256',
      SemesterProject4830::Application.credentials.secret_key_base,
      raw_token
    )
  end
end